...
접기 버튼은 본문 visibility만 바꾸지 않고 실제 slot 높이를 약 48px로 줄입니다. 펼칠 때 이전 크기를 복원하며 DPI와 화면 크기에 맞춰 위치를 보정합니다. 위치·접힘·LiDAR 표시 상태는 `Saved/SaveGames/MA0T10_VirtualSensorUI_v1.sav`에 사용자별로 저장됩니다.
