...
현재 브랜치는 가상 LiDAR/카메라, point-cloud-only view, Slab 각도 분석 v1, CSV/JSONL point cloud replay source에 초점을 둡니다. Livox/RealSense/ROS2 직접 입력은 `URealSensorSourceComp` 기반 placeholder까지 준비되어 있고, SDK/bridge 연동은 후속 작업입니다.
...
`ULidarCsvReplaySourceComp`와 `ULidarJsonLinesReplaySourceComp`는 `URealSensorSourceComp`를 상속하며, 저장된 point cloud를 `UVirtualLidarSensorComp::InjectPointCloudFrame`으로 주입합니다. 이 경로를 쓰면 파일 replay 데이터도 LiDAR JSON payload, recorder, transport, preview, Slab 분석 경로를 함께 탑니다.
...
위 컴포넌트들은 아직 SDK/bridge 연결을 수행하지 않고, 설정 필드와 명확한 not-implemented 상태 메시지만 제공합니다.
