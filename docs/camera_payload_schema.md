...
This schema is the current DT-Project contract for virtual camera frames. It is generated by `UVirtualCameraCaptureComponent::CaptureAndSendImage()` and cached in `GetLastJsonPayload()` for transport, recorder, monitor export, and debugging.
...
`UVirtualCameraCaptureComponent::GetLastJsonPayload()` returns the last JSON payload generated by `CaptureAndSendImage()`. Use it when you need to inspect the exact camera payload without changing transport mode.

External real-camera bridges can inject an already-normalized
`virtual-camera.v1` JSON payload through
`UVirtualCameraCaptureComponent::InjectExternalJsonPayload`. This path validates the core
