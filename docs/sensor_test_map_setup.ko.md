...
장비 프로필은 `아이요봇 ML-X(80) - Native`와 `아이요봇 ML-X(80) - Integration 200`으로 나뉩니다. Native FullSpec은 공개 각 해상도로 계산한 약 576×56·0.05초·15,000cm이고 `원본 사양` 배지를 표시합니다. Integration은 기존 200×56·0.05초 계약을 유지하며 `통합/다운샘플` 배지를 표시합니다. 수평 80°, 수직 -11.65°~11.65°와 최대 거리 입력 상한 20,000cm는 같습니다. 수동 편집 시 품질은 Custom으로 바뀝니다.

설정 패널의 진단에는 `공개 사양 기반 / 실장비 캘리브레이션 / Replay`, 프로토콜 검증 여부, 요청·측정·출력 Hz, deadline miss와 실제 활성 acquisition backend가 표시됩니다. ML-X SDK·캘리브레이션·원시 캡처를 대조하기 전에는 반드시 `공개 사양 기반`으로 해석하십시오.
