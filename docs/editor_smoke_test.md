...
The runner launches the normal 1920x1080 renderer, creates command-line-only FullSpec sensors, warms up for 10 seconds, resets the rolling statistics window, records 60 seconds, and writes renderer-specific JSON/Markdown under `Saved/Reports`. Select `-LidarProfile Mid360|MLX80Integration|MLX80Native` and `-LidarAcquisition Auto|Cpu|Gpu`. It verifies the requested frame shape (21,600/11,200/32,256 rays), requested/acquisition/output rates, one-job-per-sensor queue bound, and a max/min acquisition ratio of 1.2 or less. `Niagara` controls visualization only; the acquisition backend is selected independently. The temporary benchmark actors are never saved to `SensorTestMap`.

Large-scene validation uses `/Game/MA0T10/Maps/Tests/SensorScaleStressMap`. Its managed `AVirtualSensorStressSceneActor` contains 10,000 static HISM primitives and 1,000 HISM moving proxies. This fixture tests scene density and world-change handling without conflating the result with 11,000 Actor/component Tick costs. Regenerate it with `Scripts/setup_sensor_scale_stress_map.py`; the source `SensorRefactorTestMap` and user-managed maps are not modified.
