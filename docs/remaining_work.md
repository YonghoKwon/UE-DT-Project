...
- Three HostActor-managed native-backed panels for monitor, sensor settings,
  and capture/export, including drag, collapse, reset, DPI correction, and
  1280x720 compact placement.
- Runtime camera/LiDAR transform and validated setting edits with explicit
  Editor-only `SensorTestMap에 저장 예약` snapshot persistence.
- Unified payload/point-cloud/timed-capture result UI with documented storage
  roots and recent-result history.
- D455 and Mid-360S device metadata separated from the default
  `RealTimePreview` simulation budget.
...
- `Content/MA0T10/UI/WBP_VirtualSensorSettings.uasset` and
  `Content/MA0T10/UI/WBP_VirtualSensorCaptureExport.uasset`
  - Generated by `Scripts/setup_sensor_test_map.py` with the correct native
    parent classes.
  - Verify that the native-backed panels open in PIE, drag/reset/collapse work,
    and SensorTestMap HostActor class references resolve before staging.
- `Config/Game.ini`
