...
- 상단에서 Camera/LiDAR를 선택하면 설정 종류·SensorId·기즈모 대상이 함께 바뀝니다. 설정을 숨긴 상태의 선택 변경도 반영합니다. 센서 조작 중 다른 센서를 선택하면 이전 센서의 조작을 종료하며, 이전 화면으로 되돌리지 않습니다. 미등록 설정 Widget의 독립적인 선택 방식은 유지합니다.
