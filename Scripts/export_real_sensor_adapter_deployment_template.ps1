...
$template = [PSCustomObject]@{
    ...
    PreDeploymentEvidence = [PSCustomObject]@{
        ...
        HttpUdpLoopbackSmoke = "HTTP/UDP loopback smoke proves local bridge behavior only; it does not prove external deployment broker, network, topic, credential, SDK, or hardware acceptance."
        StaticTransactionRegistration = "Static DTCore transaction registration evidence proves project routing readiness only; it is not broker subscription or deployment endpoint acceptance."
        SampleValidation = "Static sample validation is not a substitute for external endpoint or hardware evidence."
    }
    DeploymentAcceptanceEvidence = [PSCustomObject]@{
        ExternalBrokerEndpointOwnership = "Owner-held endpoint reference, bind/network/auth policy, and redacted evidence path are required before claiming broker deployment acceptance."
        BrokerCredentialPolicy = "Credential handling, secret redaction, and access ownership must be recorded outside the repository."
        BrokerTopicRouting = "Deployment topic/transaction routing must be proven by a real broker PIE smoke, not by brokerless dispatch."
        BrokerPieSmoke = "A real external broker run must record map/session/log/frame-count/target-points/cached-payload/client-command evidence."
        HttpExternalDeploymentSmoke = "HTTP deployment acceptance requires external bind/route ownership and real POST evidence, not localhost-only loopback."
        UdpExternalDeploymentSmoke = "UDP deployment acceptance requires deployment bind/port ownership and datagram evidence, not localhost-only loopback."
        SdkHardwareSmoke = "ROS2/Livox/RealSense acceptance requires real SDK/device smoke evidence for the selected path."
        OwnerAcceptance = "Deployment owner must accept the selected path and all not-selected path decisions."
    }
    ...
    Summary = [PSCustomObject]@{
        ...
        ExternalBrokerSmokeEvidencePresent = $false
        ExternalSdkHardwareEvidencePresent = $false
        DeploymentBrokerAcceptanceComplete = $false
        SdkDeploymentAcceptanceComplete = $false
