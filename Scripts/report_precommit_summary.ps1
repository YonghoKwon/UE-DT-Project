...
$workAreas = @(
    ...
    (New-WorkArea `
        ...
        -Percent 66 `
        -Done "Server payload and preview policies are separated with preview caps, runtime warnings, point-cloud-only clamps, batched ISM AddInstances live preview uploads, procedural CSV high-density preview automation, instanced fallback automation, CSV preview parse/build/load telemetry, generous headless procedural performance-budget automation, exportable CSV preview performance evidence from Unreal automation logs, static preview-policy validation, and a high-density renderer decision report covering CPU fallback, Niagara, custom GPU buffers, and external viewer workflows. The renderer decision report consumes local CSV preview performance evidence, separates ISM smoke from procedural dense evidence, rejects failure lines inside the selected automation run block, records decision gates, and now recommends a Niagara point-renderer spike while preserving CPU preview fallback." `
        -Remaining "Niagara spike implementation, viewport smoke evidence, renderer-specific dense-frame performance validation, and fallback-preservation verification after GPU integration remain."),
    (New-WorkArea `
        -Name "LAZ export" `
        -Percent 54 `
