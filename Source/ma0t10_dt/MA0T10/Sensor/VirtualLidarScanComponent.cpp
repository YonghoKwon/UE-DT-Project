...
FString BuildScheduledLidarJson(const FLidarScheduledPayloadSnapshot& S)
{
    ...
    for (int32 I = 0; I < Points.Num(); I += SafeStride)
    {
        ...
        O->SetNumberField(TEXT("rangeMillimeters"), P.RangeMillimeters);
        O->SetNumberField(TEXT("intensity"), P.RawIntensity);
        O->SetNumberField(TEXT("intensityNormalized"), P.NormalizedIntensity);
        O->SetNumberField(TEXT("ring"), P.Ring);
        O->SetNumberField(TEXT("horizontalIndex"), P.HorizontalIndex);
        O->SetNumberField(TEXT("echoIndex"), P.EchoIndex);
        O->SetNumberField(TEXT("echoCount"), P.EchoCount);
        O->SetStringField(TEXT("pointTimeOffsetNanoseconds"), LexToString(P.PointTimeOffsetNanoseconds));
        O->SetNumberField(TEXT("validity"), static_cast<int32>(P.Validity));
	...
	float InMaxDistanceCm)
{
    ...
    Snapshot->AcquisitionBackend = AcquisitionBackend == EVirtualLidarAcquisitionBackend::Auto
